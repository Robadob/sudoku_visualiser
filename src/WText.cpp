...
WText::WText(const wchar_t *string, unsigned int fontHeight, glm::vec3 color, char const *fontFile, unsigned int faceIndex)
    :WText(string, fontHeight, glm::vec4(color, 1.0f), fontFile, faceIndex) {}
WText::WText(const wchar_t *_string, unsigned int fontHeight, glm::vec4 color, char const *fontFile, unsigned int faceIndex)
    ...
    error =  FT_Select_Charmap(this->font, ft_encoding_unicode);  // ft_encoding_symbol ft_encoding_unicode
    if (error) {
        fprintf(stderr, "An unexpected error occurred whilst switching to unicode charmap %s: %i\n", fontFile, error);
    }
...
void WText::recomputeTex() {
    ...
    for (unsigned int n = 0; n < stringLen; n++) {
        ...
        if (glyphs[i].index == 0)
            printf("No glypth found for char '%ls'\n", string);
        ...
        if (!(glyphs[i].c == '\n' || glyphs[i].c == '\r')) {
            ...
            if (!error) {
                ...
                if (penX >= 0 && penX + bit->bitmap.pitch <= static_cast<int>(texDim.x) && penY >= 0 && penY + static_cast<int>(bit->bitmap.rows) <= static_cast<int>(texDim.y)) {
                    ...
                    fprintf(stderr, "Skipped painting char '%c' of '%ls' to avoid writing out of bounds.%i\n", glyphs[i].c, string, static_cast<int>(FT_IS_SCALABLE(this->font)));
                }
            }
        }
...
void WText::setStringLen() {
    stringLen = 0;
    //  ReSharper disable once CppPossiblyErroneousEmptyStatements
    while (string[stringLen++] != '\0') { }
    stringLen--;
}
void WText::setFontHeight(unsigned int pixels, bool refreshTex) {
    if (!font) {
...
unsigned int WText::getFontHeight() const {
    return this->fontHeight;
}
void WText::setPadding(unsigned int _padding, bool refreshTex) {
    this->padding = _padding;
    if (refreshTex)
        recomputeTex();
}
unsigned int WText::getPadding() const {
    return this->padding;
}
/*
...
*/
void WText::setUseAA(bool aa, bool refreshTex) {
    this->printMono = !aa;
    if (refreshTex)
        recomputeTex();
}
bool WText::getUseAA() const {
    return !this->printMono;
}
void WText::setMaxWidth(unsigned int maxWidth, bool refreshTex) {
    this->wrapDistance = maxWidth;
    if (refreshTex)
        recomputeTex();
}
unsigned int WText::getMaxWidth() const {
    return this->wrapDistance;
}
void WText::setLineSpacing(float _lineSpacing, bool refreshTex) {
...
void WText::setColor(glm::vec3 _color) {
    setColor(glm::vec4(_color, 1.0f));
}
void WText::setColor(glm::vec4 _color) {
    this->color = _color;
    getShaders()->addStaticUniform("_col", glm::value_ptr(this->color), 4);
}
void WText::setBackgroundColor(glm::vec3 _color) {
    setBackgroundColor(glm::vec4(_color, 1.0f));
}
void WText::setBackgroundColor(glm::vec4 _color) {
    this->backgroundColor = _color;
    getShaders()->addStaticUniform("_backCol", glm::value_ptr(this->backgroundColor), 4);
}
glm::vec4 WText::getColor() const {
    return color;
}
glm::vec4 WText::getBackgroundColor() const {
    return backgroundColor;
}
void WText::setString(const wchar_t*fmt, ...) {
    if (this->string)
        delete this->string;
    int bufSize = 0;
    int ct = 0;
    ...
    do {  // Repeat until buffer is large enough
        if (buffer)
            free(buffer);
        bufSize += 128;
        buffer = reinterpret_cast<wchar_t*>(malloc(bufSize * sizeof(wchar_t)));
        ct = vswprintf(buffer, bufSize, fmt, argp);
    } while (ct == -1);
    va_end(argp);
    this->string = buffer;
    recomputeTex();
}
WText::TextureString::TextureString()
...
void WText::TextureString::resize(const glm::uvec2 &_dimensions) {
...
void WText::TextureString::updateTex() {
    if (!texture)
        return;
...
void WText::TextureString::paintGlyph(FT_Bitmap glyph, unsigned int penX, unsigned int penY) {
...
void WText::TextureString::paintGlyphMono(FT_Bitmap glyph, unsigned int penX, unsigned int penY) {
    for (unsigned int y = 0; y < glyph.rows; y++) {
        // src ptr maps to the start of the current row in the glyph
        unsigned char *src_ptr = glyph.buffer + y*glyph.pitch;
        // dst ptr maps to the pens current Y pos, adjusted for the current glyph row
